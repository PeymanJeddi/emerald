...
export default function CMSAboutPage() {
  ...
  function load() {
    apiFetch<Section[]>("/api/admin/cms/sections?page_key=about").then(setSections);
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4">
        <h1 className="font-serif text-3xl text-emerald">About Page CMS</h1>
        <Link href="/cms" className="text-sm text-emerald-accent hover:underline">
          Back to CMS
        </Link>
      </div>
      <p className="mt-2 text-sm text-graphite/60">
        Edit hero, mission, statistics, timeline, partners, gallery, and CTA sections. Testimonials are managed separately.
      </p>
        ...
        {sections.map((s) => (
          ...
          >
            <div>
              ...
              <p className="mt-1 line-clamp-2 text-sm text-graphite/60">{s.subtitle || s.body || "—"}</p>
            </div>
          ...
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-sm border border-border bg-ivory p-6 shadow-lg">
            ...
            <div className="mt-4 space-y-3">
              ...
              <textarea
                ...
                placeholder="Body (markdown-style paragraphs separated by blank lines)"
                rows={5}
                className="w-full rounded-sm border border-border px-3 py-2 text-sm"
              />
              <label className="block text-xs font-medium text-graphite/60">config_json (cards, bullets, stats, etc.)</label>
              <textarea
                value={jsonDraft}
                onChange={(e) => setJsonDraft(e.target.value)}
                rows={12}
                className="w-full font-mono text-xs rounded-sm border border-border px-3 py-2"
              />
            </div>
            ...
            <div className="mt-6 flex gap-3">
              <button
                ...
                onClick={save}
                disabled={saving}
                className="rounded-sm bg-emerald px-4 py-2 text-sm text-ivory disabled:opacity-50"
              >
                {saving ? "Saving…" : "Save"}
              </button>
